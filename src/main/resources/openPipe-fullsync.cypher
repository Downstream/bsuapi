...
MATCH (t:Topic) SET t.artCount = 0
...
SET r.hasGoodImages =
  (CASE WHEN exists(x.primaryImageSmall) THEN 2 ELSE 0 END)
+ (CASE WHEN exists(x.primaryImageLarge) THEN 1 ELSE 0 END)
RETURN 'Indentifying Artist representative images' AS t
;

OPTIONAL MATCH (a:Artist)<-[r:BY]-(:Asset)
  WHERE r.representative > 0
WITH a, r ORDER BY r.hasGoodImages DESC, r.representative DESC
